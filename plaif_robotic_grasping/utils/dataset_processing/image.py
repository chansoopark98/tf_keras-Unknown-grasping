...
class Image:
    ...
    def rotate(self, angle, center=None):
        ...
        self.img = rotate(self.img, angle / np.pi * 180, center=center, mode='edge', preserve_range=True).astype(
            self.img.dtype)
    ...
    def pad(self, p_w0, p_w1, p_h0, p_h1):
        if len(self.img.shape) == 2:
            padding_width = ((p_h0, p_h1),(p_w0, p_w1))
        else:
            padding_width = ((p_h0, p_h1),(p_w0, p_w1),(0,0))
        self.img = pad(self.img, padding_width, mode='edge').astype(
            self.img.dtype)
    ...
    def augment(self):
        ...
        seq = iaa.Sequential(
            [sometimes(iaa.OneOf([iaa.Invert(0.1),iaa.ChannelShuffle(0.35)])),
             iaa.OneOf([iaa.Add((-20, 20), per_channel=0.2), iaa.Multiply((0.5, 1.5), per_channel=0.2)])],
            random_order=True
        )
        img_aug = seq(images=np.expand_dims(self.img, 0))
        self.img = img_aug[0]

    def augmented(self, *args, **kwargs):
        i = self.copy()
        i.augment(*args, **kwargs)
        return i
...
class DepthImage(Image):
    ...
    def normalise(self):
        ...
        """
        if (self.img.dtype == 'uint16') | (self.img.dtype == 'int32'):
            self.img = self.img.astype(np.float32) * 0.001
