...
class GraspRectangles:
    ...
    @classmethod
    def load_from_cornell_file(cls, fname, scale=1.0, p_w0=0, p_h0=0):
        """
    ...
    @classmethod
    def load_from_plaif_file(cls, fname):
        """
        Load grasp rectangles from a Jacquard dataset file.
        :param fname: Path to file.
        :param scale: Scale to apply (e.g. if resizing images)
...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1, scale=1.0):
    """
    ...
    """
    local_max = peak_local_max(q_img, min_distance=20, threshold_abs=0.3, threshold_rel=0.6, num_peaks=no_grasps)
    ...
    for grasp_point_array in local_max:
        ...
        scaled_grasp_point = (int(grasp_point[0]*scale), int(grasp_point[1]*scale))
        g = Grasp(scaled_grasp_point, grasp_angle)
        if width_img is not None:
            g.length = width_img[grasp_point] * scale
            g.width = g.length / 2
...
def detect_grasps_nframes(q_img, ang_img, width_img=None, no_grasps=1, scale=1.0):
    """
    Detect grasps in a network output.
    :param q_img: Q image network output
    :param ang_img: Angle image network output
    ...
    :return: list of Grasps
    """
    local_max = peak_local_max(np.max(q_img, axis=0), min_distance=20, threshold_abs=0.25, threshold_rel=0.6, num_peaks=no_grasps)
    ...
    for grasp_point_array in local_max:
        ...
        _grasp_q = q_img[:, grasp_point[0], grasp_point[1]]
        q_order = np.argsort(_grasp_q)
        _grasp_angle = ang_img[:, grasp_point[0], grasp_point[1]]
        grasp_angle = _grasp_angle[q_order[-1]]

        g = Grasp(scaled_grasp_point, grasp_angle)
        if width_img is not None:
            _grasp_width = width_img[:, grasp_point[0], grasp_point[1]]
            g.length = _grasp_width[q_order[-1]] * scale
            g.width = g.length / 2
