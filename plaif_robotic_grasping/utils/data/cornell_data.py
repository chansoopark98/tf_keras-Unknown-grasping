...
class CornellDataset(GraspDatasetBase):
    ...
    def get_gtbb(self, idx, rot=0, zoom=1.0):
        gtbbs = grasp.GraspRectangles.load_from_cornell_file(self.grasp_files[idx], p_w0=16, p_h0=96)
    ...
    def get_rgb(self, idx, rot=0, zoom=1.0, normalise=True, augment=False):
        ...
        # rgb_img.pad(0, 0, 80, 80)
        rgb_img.pad(16, 16, 96, 96)
        rgb_img.rotate(rot)
        rgb_img.zoom(zoom)
        # cv2.imwrite("/root/workspace/robotic-grasping/utils/data/test_zoom.png", rgb_img.img)
        # rgb_img.resize((self.output_size, self.output_size))
        # cv2.imwrite("/root/workspace/robotic-grasping/utils/data/test_resize.png", rgb_img.img)
        if normalise:
            rgb_img.normalise()
