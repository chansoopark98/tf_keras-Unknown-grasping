...
from src.robotic_grasping.utils.dataset_processing.grasp import detect_grasps
...
def save_results(rgb_img, grasp_q_img, grasp_angle_img, depth_img=None, no_grasps=1, grasp_width_img=None):
    ...
    :return:
    """
    results = "/root/workspace/robotic-grasping/results"
    gs = detect_grasps(grasp_q_img, grasp_angle_img, width_img=grasp_width_img, no_grasps=no_grasps)
...
def save_results_with_name(path, data_id, rgb_img, grasp_q_img, grasp_angle_img, depth_img=None, no_grasps=1, grasp_width_img=None):
    """
    Plot the output of a network
    :param rgb_img: RGB Image
    :param depth_img: Depth Image
    ...
    s_time = time.time()
    gs = detect_grasps(grasp_q_img, grasp_angle_img, width_img=grasp_width_img, no_grasps=no_grasps)
    print(f'grasp detecting time: {time.time()-s_time}')
    ...
    ax.set_title('RGB')
    ax.axis('off')
    fig.savefig(f'{results}/{data_id:04}_rgb.png')
    ...
    if depth_img.any():
        ...
        ax.set_title('Depth')
        ax.axis('off')
        fig.savefig(f'{results}/{data_id:04}_depth.png')
    ...
    ax.set_title('Grasp')
    ax.axis('off')
    fig.savefig(f'{results}/{data_id:04}_grasp.png')
    ...
    ax.axis('off')
    plt.colorbar(plot)
    fig.savefig(f'{results}/{data_id:04}_quality.png')
    ...
    ax.axis('off')
    plt.colorbar(plot)
    fig.savefig(f'{results}/{data_id:04}_angle.png')
    ...
    ax.axis('off')
    plt.colorbar(plot)
    fig.savefig(f'{results}/{data_id:04}_width.png')
